        ...
        if(player.isShiftKeyDown()){
            ...
            if(nbt.getInt("state")==4){
                ...
                switch (nbt.getInt("state")){
                    case 1: player.displayClientMessage(new StringTextComponent(String.format("Switched to Fire Sim Mode")), true); break;
                    case 2: player.displayClientMessage(new StringTextComponent(String.format("Switched to Remove Sim From Faction Mode")), true); break;
                    case 3: player.displayClientMessage(new StringTextComponent(String.format("Switched to Reset Targeted Sim's Inventory")), true); break;
                    case 4: player.displayClientMessage(new StringTextComponent(String.format("Switched to Purge Bugged Sims Mode")), true); break;
                }
            }
            item.setTag(nbt);
        }
            ...
            if(nbt.getInt("state") == 4){
                ...
                ArrayList<Integer> simList = faction.getSimIds((ServerWorld) world);
    ...
    public ActionResultType interactLivingEntity(ItemStack item, PlayerEntity player, LivingEntity entity, Hand hand) {
        ...
        if (entity instanceof SimEntity && !player.isShiftKeyDown()) {
            ...
            switch(item.getOrCreateTag().getInt("state")){
                //kills targeted sim
                case 0: ((SimEntity) entity).die(DamageSource.OUT_OF_WORLD); SavedWorldData.get(entity.level).getFactionWithSim(((SimEntity) entity).getUUID()); break;
                //fires targeted sim
                case 1: ((SimEntity) entity).fireSim((SimEntity) entity, SavedWorldData.get(entity.level).getFactionWithSim(entity.getUUID()).getId(),false); break;
                //removes targeted sim from faction
                case 2: SavedWorldData.get(player.level).getFactionWithSim(entity.getUUID()).removeSim(entity.getUUID()); break;
                //resets targeted sim's inventory
                case 3: ((SimEntity) entity).resetInventory(); break;
            }
